...
function StoryCard(props: IStoryCardProps) {
  const { story } = props
  const { isAuthenticated, authToken, user } = useSelector(getAuth)
  ...
  return (
    <div className={`${style['story-card']}`}>
      <Link to={generatePath(AppRoutes.STORY, { id: story.id })} className={style['header']}>
        ...
        <div className={style['title']}>{story.info.title}</div>
        <div className={style['desc']}>
          {StringHelper.truncateTextWithEllipsis(story.info.description, 172)}
        </div>
      </Link>
